...
注意打包时如果通过获取目录下文件的方式来做的话，记得要先处理路径，去除meta文件，unity打AB包的本质是把资源导入unity时unity生成的他自己的资源格式打成包（Library/Artifacts）Unity通过GUID和相对目录获取导入后的资源，基本可以通过AssetDatabase函数来把资源校验做完。有些不需要打进包的文件记得去除。